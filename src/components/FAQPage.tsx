        ...
        {faqItems.map((item, i) => (
          <div key={i} className="glass-card rounded-xl overflow-hidden">
            ...
            >
              ...
              <ChevronDown className={`w-5 h-5 text-[#F61A5A] shrink-0 transition-transform duration-300 ${openIndex === i ? 'rotate-180' : ''}`} />
            </button>
            <div className={`overflow-hidden transition-all duration-300 ${openIndex === i ? 'max-h-60 opacity-100' : 'max-h-0 opacity-0'}`}>
              <p className="px-5 pb-5 text-sm text-[#8888A0] leading-relaxed">{t(item.aKey, currentLang)}</p>
            </div>
          </div>
        ))}
      </div>
