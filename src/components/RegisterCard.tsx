...
export default function RegisterCard() {
  ...
  const handleRegister = async (e: React.FormEvent) => {
    ...
    if (password !== confirmPassword) {
      setError(t('passwordMismatch', currentLang));
      return;
    }
    if (!agreeTerms) {
      setError(t('agreeTerms', currentLang));
      return;
    }
    ...
    try {
      ...
      if (res.ok) {
        ...
        setError(data.error || t('error', currentLang));
      }
    } catch {
      ...
      <div className="w-full max-w-md">
        <div className="glass-card rounded-2xl p-8">
          <div className="text-center mb-8">
            ...
            <h2 className="text-2xl font-bold text-white">{t('createAccount', currentLang)}</h2>
          </div>
          ...
          <form onSubmit={handleRegister} className="space-y-4">
            <div className="relative">
              <User className={`absolute top-1/2 -translate-y-1/2 w-4 h-4 text-[#8888A0] ${isRTL ? 'right-3' : 'left-3'}`} />
              <input
                type="text"
                value={name}
                onChange={(e) => setName(e.target.value)}
                placeholder={t('name', currentLang)}
                required
            ...
            <div className="relative">
              ...
              <input
                ...
                placeholder={t('confirmPassword', currentLang)}
                required
                className="w-full py-3 rounded-xl bg-white/5 border border-white/10 text-white text-sm placeholder:text-[#8888A0] focus:outline-none focus:border-[#F61A5A]/50"
                dir="ltr"
                style={{ paddingInlineStart: '2.5rem', paddingInlineEnd: '0.75rem' }}
            ...
            <label className="flex items-start gap-2 cursor-pointer">
              <input
                ...
                className="rounded accent-[#F61A5A] mt-0.5"
              />
              <span className="text-xs text-[#8888A0]">{t('agreeTerms', currentLang)}</span>
            </label>
            ...
            >
              {loading ? t('loading', currentLang) : t('createAccount', currentLang)}
            </button>
          </form>
